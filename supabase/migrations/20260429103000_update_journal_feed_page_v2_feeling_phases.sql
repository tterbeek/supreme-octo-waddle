...
create function public.get_journal_feed_page_v2(p_limit integer, p_offset integer)
returns table(
  entry_kind text,
  day date,
  within_day_ts timestamp with time zone,
  id uuid,
  user_id uuid,
  type text,
  date date,
  ...
  entry_type text,
  entry_text text,
  journal_created_at timestamp with time zone,
  metadata jsonb,
  equipment jsonb,
  photos jsonb,
  source text,
...
journal_rows as (
  select
    ...
    null::text as feeling_during,
    null::text as feeling_after,
